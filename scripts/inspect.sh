...
function check_service {
	...
	if is_strict; then
		...
		if ! snapctl services $service | grep inactive &>/dev/null; then
			status="active"
		fi
	else
...
function check_apparmor {
	...
	if is_strict; then
		...
		if [ -f /etc/apparmor.d/containerd ]; then
...
function suggest_fixes {
	...
	status="inactive"
	if is_strict; then
		if ! snapctl services microk8s.daemon-kubelite | grep inactive &>/dev/null; then
			status="active"
		fi
	else
		if systemctl status snap.microk8s.daemon-kubelite &>/dev/null; then
			status="active"
		fi
	fi

	if [ "$status" == "inactive" ]; then
		if lsof -Pi :16443 -sTCP:LISTEN -t &>/dev/null; then
			printf -- '\033[0;33m WARNING: \033[0m Port 16443 seems to be in use by another application.\n'
		fi
	fi

	if iptables -L 2>&1 | grep FORWARD | grep DROP &>/dev/null; then
	...
	if getenforce 2>&1 | grep 'Enabled' >/dev/null; then
	...
	if [ -d "/etc/docker/" ] && ! [ -z "$(which dockerd)" ]; then
		...
		# else if the file docker/daemon.json exists
			...
			if ! grep -qs localhost:32000 /etc/docker/daemon.json; then
				printf -- '\033[0;33mWARNING: \033[0m Docker is installed. \n'
	...
	if ! is_strict && fedora_release; then
		...
		if [ ! -d "/sys/fs/cgroup/memory" ] &>/dev/null; then
	...
	# Debian 9 checks
	if ! is_strict && debian9_release; then

		# Check if snapctl is fresh
		if ! [ -L "/usr/bin/snapctl" ]; then
	...
	# LXD Specific Checks
	if ! is_strict && cat /proc/1/environ | grep "container=lxc" &>/dev/null; then

		# make sure the /dev/kmsg is available, indicating a potential missing profile
		if [ ! -c "/dev/kmsg" ]; then # kmsg is a character device
	...
	# node name
	nodename="$(hostname)"
	if [[ "$nodename" =~ [A-Z|_] ]] && ! grep -e "hostname-override" "${SNAP_DATA}/args/kubelet" &>/dev/null; then
	...
	if grep Raspberry /proc/cpuinfo -q &&
		...
		! dpkg -l | grep linux-modules-extra-raspi -q; then
...
function debian9_release {
	if [ -e /etc/debian_version ] &&
		grep "9\." /etc/debian_version -q; then
		return 0
	else
		return 1
	fi
...
function check_low_memory_guard {
	if [ -e "${SNAP_DATA}/var/lock/low-memory-guard.lock" ]; then
...
function check_hostname {
	HOST=$(hostname)
	if echo "${HOST}" | grep -q "[A-Z]" 2>/dev/null; then
...
if ! [ -e "${SNAP_DATA}/var/lock/ha-cluster" ]; then
...
if ! [ -e "${SNAP_DATA}/var/lock/no-apiserver-proxy" ]; then
