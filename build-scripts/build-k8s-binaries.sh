...
(cd $GOPATH/src/$KUBERNETES_REPOSITORY
  ...
  do
    if [ "$app" = "cmd/kubelite" ]
    then
      make WHAT="${app}" GOFLAGS=-tags=libsqlite3,dqlite CGO_CFLAGS="-I${SNAPCRAFT_STAGE}/usr/include/" CGO_LDFLAGS="-L${SNAPCRAFT_STAGE}/lib" CGO_LDFLAGS_ALLOW="-Wl,-z,now" KUBE_CGO_OVERRIDES=kubelite
    else
      make WHAT="${app}"
    fi
  done
